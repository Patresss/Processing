...
class CameraHandler(
    ...
    fun getTouchedInsect(insects: ArrayList<Insect>): List<Insect> {
        val aliveInsects = insects.filter { it.alive }
        val insectsPixelsMap = HashMap(aliveInsects.associateBy({ it }, { 0 }))
        ...
        return aliveInsects.filter { insectsPixelsMap[it]!!.toDouble() / it.getNumberOfPixels().toDouble() > acceptableCover }
    }
    ...
    private fun drawImageFromCamera() {
        ...
        if (backgroundMode) {
            ...
            addTransparentDiff(cameraImage)
            pApplet.image(cameraImage, 0f, 0f, SketchLab06.CAMERA_RESOLUTION_WIDTH * SketchLab06.SCALE, SketchLab06.CAMERA_RESOLUTION_HEIGHT * SketchLab06.SCALE)
        }
    ...
    private fun drawImageWithBackground(image: PImage) {
        ...
        pApplet.image(image, 0f, 0f, SketchLab06.CAMERA_RESOLUTION_WIDTH * SketchLab06.SCALE, SketchLab06.CAMERA_RESOLUTION_HEIGHT * SketchLab06.SCALE)
    }
